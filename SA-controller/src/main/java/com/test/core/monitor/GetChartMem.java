...
@ServerEndpoint("/getMem/{root}/{node}")
public class GetChartMem {
...
//		session.getAsyncRemote().sendObject(cpu);
	...
	public static void sendMessageToWeb(Session session,ServerData data) throws IOException, EncodeException {
		session.getBasicRemote().sendObject(data.getMemoryPercent());
	}
